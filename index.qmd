...
Principal component analysis is a method of analyzing a data set that can reduce a large number of variables. Principal component analysis takes two or more variables and combines them into one principal component. A principal component is a linear combination of these variables as a unit vector. The weights of each variables are considered, and the linear combination accounts for the weight of each variable [@bro]. The variables are weighted by using eigenvectors.
...
AGE and MPG: There is a strong positive correlation between AGE and MPG. This means that as cars get older, they tend to get worse gas mileage.

GP and MPG: There is a strong negative correlation between GP and MPG. This means that cars with more horsepower tend to get worse gas mileage.

MINpct and MPG: There is a strong negative correlation between MINpct and MPG. This means that cars with a higher percentage of minutes spent idling tend to get worse gas mileage.

USGpct and MPG: There is a strong negative correlation between USGpct and MPG. This means that cars with a higher percentage of time spent under hard acceleration tend to get worse gas mileage.

TOPG and MPG: There is a strong negative correlation between TOPG and MPG. This means that cars with a higher percentage of top speed runs tend to get worse gas mileage.

FTA and Ftpct: There is a strong positive correlation between FTA and Ftpct. This means that cars with a higher percentage of faults detected during automatic testing tend to have a higher percentage of faults detected during manual testing.

Tspct and PPG: There is a strong positive correlation between Tspct and PPG. This means that cars with a higher percentage of time spent at a constant speed tend to have a higher points per gallon rating.

MPG and PPG: There is a strong positive correlation between MPG and PPG. This means that cars with better gas mileage tend to have a higher points per gallon rating.
...
ggplot(top_10_points_players, aes(x = reorder(NAME, -PPG), y = PPG, fill = NAME)) +
  ...
  scale_fill_brewer(palette = "Set4") +  # Using the same custom color palette
  theme_minimal()
```
...
ggplot(top_10_3p_players, aes(x = reorder(NAME, -pct3P), y = pct3P, fill = NAME)) +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  scale_fill_brewer(palette = "Set4") +  # Using Set4 color palette
  theme_minimal()

```

The above graph shows the ten players with the most three-pointers per game.
